...
(function() {
  ...
  var SYSTEM_PROMPT = [
    ...
    '',
    '## RULES:',
    '- 4 sections: RECOMMENDED (max 3), UPI, CARDS, MORE WAYS TO PAY',
    '- Each instrument appears in EXACTLY ONE section. NO duplicates.',
    '- UPI-type instruments (apay_upi, other_upi) can ONLY go in "upi" OR "recommended" (if they have a badge). Never both.',
    '- Card-type instruments can ONLY go in "cards" OR "recommended" (if they have a badge). Never both.',
    '- apay_balance, apay_later, cod, emi, net_banking go in "more_ways" OR "recommended" (if they have a badge). Never both.',
    '- Each badge type (Best offer, Previously used, Featured) used AT MOST ONCE.',
    '- If user says "no best offer" or similar, do NOT assign Best offer badge to anyone.',
    '- If user specifies cashback amount (e.g. "₹20 cashback"), set offerAmount to that number.',
    ...
    '- apay_later: Amazon Pay Later (bnpl)',
    '- cod: Cash on Delivery',
    '- emi: EMI',
    '- net_banking: Net Banking',
    '',
    '## STATES: normal, disabled (expired/blocked), insufficient (APB only)',
    '',
