...
(function() {
  ...
  var SYSTEM_PROMPT = [
    ...
    '- UPI-type instruments (apay_upi, other_upi) go in "upi" OR "recommended" (with badge). Never both.',
    '- Card-type instruments go in "cards" OR "recommended" (with badge). Never both.',
    '- wallet/bnpl/cod/emi/netbanking go in "more_ways" OR "recommended" (with badge). Never both.',
    '- Each badge type (Best offer, Previously used, Featured) used AT MOST ONCE.',
    '- "in featured" or "in recommended" means: put in recommended section WITH the Featured badge.',
    '- "in previously used" means: put in recommended section WITH the Previously used badge.',
    '- If user says "no best offer", do NOT assign Best offer badge.',
